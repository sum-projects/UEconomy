...
public class MainGame : Game
{
    ...
    private void DrawPriceChart(int x, int y, int width, int height, string resource)
    {
        ...
        var history = _priceHistory[resource];
        ...
        _spriteBatch.DrawString(
            _font,
            $"Obecna: {_market.GetPrice(resource):F2}$",
            new Vector2(x + width - 10, y + height - 20),
            GetResourceColor(resource),
            0f,
            new Vector2(_font.MeasureString($"Obecna: {_market.GetPrice(resource):F2}$").X, 0),
            _fontScale * 0.8f,
            SpriteEffects.None,
            0f
        );
    ...
    protected override void Draw(GameTime gameTime)
    {
        ...
        foreach (var province in _provinces)
        {
            ...
            DrawGradientCell(cellX, cellY, _cellSize - 2, _cellSize - 2, provinceColor);

            // Jeśli prowincja jest wybrana, dodaj efektowne obramowanie
            if (_selectedProvince != null && _selectedProvince.Id == province.Id)
            {
                DrawSelectedBorder(cellX, cellY, _cellSize, _cellSize);
            }
            ...
            if (population != null)
            {
                ...
                // Rysuj liczbę mieszkańców w elegancki sposób
                _spriteBatch.DrawString(
                    _font,
                    $"{population.Size}",
    ...
    private void DrawGlobalStatsPanel()
    {
        ...
        int panelX = _gridMarginX + _gridWidth * _cellSize + 20;
        int panelY = _gridMarginY;
        int panelWidth = (int)(GraphicsDevice.Viewport.Width * 0.24f); // ~24% szerokości ekranu
        int panelHeight = _gridHeight * _cellSize;
        ...
        DrawStatsSection(panelX, ref yPos, panelWidth, "Ceny rynkowe:", sectionMargin, () =>
        {
            foreach (var resource in _market.GetAllResources())
            {
                ...
                float ratio = _market.GetSupplyDemandRatio(resource);
                ...
                _spriteBatch.DrawString(
                    _font,
                    $"{resource}: {_market.GetPrice(resource):F2}$ ({ratio:F2})",
        ...
        DrawStatsSection(panelX, ref yPos, panelWidth, "Populacja:", sectionMargin, () =>
        {
            _spriteBatch.DrawString(
                _font,
                $"{_globalStats.GetValueOrDefault("Total Population"):F0}",
                new Vector2(panelX + sectionMargin + 10, yPos),
                Color.White,
                0f,
                Vector2.Zero,
        ...
        DrawStatsSection(panelX, ref yPos, panelWidth, "Całkowite zasoby:", sectionMargin, () =>
        {
            foreach (var resource in _market.GetAllResources())
            {
                ...
                float amount = _globalStats.GetValueOrDefault($"Total {resource}");
        ...
        DrawStatsSection(panelX, ref yPos, panelWidth, "Globalna produkcja (na turę):", sectionMargin, () =>
        {
            foreach (var resource in _market.GetAllResources())
            {
                float production = _globalStats.GetValueOrDefault($"Production {resource}");
        ...
        DrawStatsSection(panelX, ref yPos, panelWidth, "Ostatnia wymiana handlowa:", sectionMargin, () =>
        {
            ...
            {
                float amount = _totalTraded[resource];
        ...
        DrawStatsSection(panelX, ref yPos, panelWidth, "Średnie zadowolenie potrzeb:", sectionMargin, () =>
        {
            foreach (var resource in _market.GetAllResources())
            {
                ...
                {
                    float satisfaction = _globalStats[$"Satisfaction {resource}"];
        ...
        AddChartAndResourceButtonsToPanel(ref yPos, panelX, panelWidth);
    }
    ...
    private void CalculateUIScaling()
    {
        ...
        _cellSize = Math.Min(availableWidth / 10, screenHeight / 8); // Maksymalnie 10x8 prowincji
        _gridWidth = availableWidth / _cellSize;
        _gridHeight = screenHeight / _cellSize;

        // Ustaw marginesy, aby siatka była wycentrowana
        _gridMarginX = (availableWidth - (_gridWidth * _cellSize)) / 2;
        _gridMarginY = (screenHeight - (_gridHeight * _cellSize)) / 2;
    ...
    {
        var iconSize = _cellSize / 10;
        var margin = 2;
        var iconX = cellX + _cellSize - iconSize - margin;
...
// Poprawiona metoda obsługi kliknięć na przyciski zasobów
    private void HandleResourceButtonClick(Point mousePosition)
    {
        ...
        string[] resources = _market.GetAllResources();
...
// Zaktualizowana część metody DrawGlobalStatsPanel dotycząca wykresu i przycisków zasobów
    private void AddChartAndResourceButtonsToPanel(ref int yPos, int panelX, int panelWidth)
    {
        // Po wyświetleniu wszystkich statystyk, dodaj wykres cen
        if (_priceHistory.ContainsKey(_selectedResource))
        {
            int chartMargin = 15;
